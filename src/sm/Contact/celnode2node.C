...
Node2NodeContact :: Node2NodeContact(int num, Domain *d, ContactDefinition *cDef, ContactPair *cPair) : StructuralContactElement(num, d, cDef, cPair)
{   
...
Node2NodeContactL2 :: Node2NodeContactL2(int num, Domain *d, ContactDefinition *cDef, ContactPair *cPair) : 
  StructuralContactElementLagrange(num, d, cDef, cPair)   
{
    this->numberOfDofMans = 2;
}
...
void
Node2NodeContactL2 :: setupIntegrationPoints()
{
    // Sets up the integration rule array which contains all the necessary integration points
    if ( this->integrationRule == NULL ) {
        this->integrationRule = new GaussIntegrationRule(1, this) ;
